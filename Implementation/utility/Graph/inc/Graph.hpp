...
public: 
    ...
    void addEdge(uint32_t vertex,uint32_t newNanosecond,uint32_t newPrice);
    ...
    inline std::unordered_map<uint32_t,Value>::iterator getMapBegin(){
        return map->begin();
    }

    inline std::unordered_map<uint32_t,Value>::iterator getMapEnd(){
        return map->end();
    }
